...
colunas1 = ['Codigo Cliente','Cliente Varejo','Produto','Qtde','Desc Produto','Filial','Desc Cor Produto','Dt_Venda']



@st.cache(allow_output_mutation=True)
def get_data():
    return pd.read_excel(arquivo1, dtype={'Codigo Cliente':object},  usecols = colunas1,engine='openpyxl')[colunas1]

total= get_data()
#total['Dt_Venda'] = total['Dt_Venda'].dt.strftime('%d-%m-%Y')
df = get_data()

df = df.groupby(['Codigo Cliente','Produto'])['Qtde'].sum()
df = df.reset_index()

cliente_pivot = df.pivot_table(index = 'Codigo Cliente', columns = 'Produto', values = 'Qtde')
...
st.sidebar.header('Historico Compras do Cliente')

# historico de compras do cliente selecionado

st.sidebar.table(total[df_filtro][['Filial','Desc Produto','Desc Cor Produto','Dt_Venda']])
...
if cliente_pivot[cliente_pivot.index == cod_cliente].shape[0] > 0:
    ...
    objetivo = total[total['Codigo Cliente'] == cod_cliente]
    ...
    for i in clientes_similares:
        similar = total[total['Codigo Cliente'] == i]
